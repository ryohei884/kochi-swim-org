...
import { approve, getByIdAdmin } from "@/lib/record/actions";
...
export default function RecordExcludeForm(props: Props) {
  ...
  const form = useForm<recordWithUserSchemaType>({
    resolver: zodResolver(recordWithUserSchema),
  ...
  const onError: SubmitErrorHandler<recordWithUserSchemaType> = (errors) => {
    toast("エラーが発生しました。", {

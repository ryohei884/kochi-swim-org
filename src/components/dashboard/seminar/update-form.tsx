...
export default function SeminarUpdateForm(props: Props) {
  ...
  const form = useForm<seminarUpdateOnSubmitSchemaType>({
    resolver: zodResolver(seminarUpdateOnSubmitSchema),
    defaultValues: seminarUpdateOnSubmitSchemaDV,
  });
  ...
  const onError: SubmitErrorHandler<seminarUpdateOnSubmitSchemaType> = async (
    errors,
  ) => {
    toast("エラーが発生しました。", {

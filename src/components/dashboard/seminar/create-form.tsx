...
export default function SeminarCreateForm(props: Props) {
  ...
  const form = useForm<seminarCreateOnSubmitSchemaType>({
    resolver: zodResolver(seminarCreateOnSubmitSchema),
    defaultValues: seminarCreateOnSubmitSchemaDV,
  });
      ...
      props.map(async (prop) => {
        if (
          ...
          const response = await fetch(
            `/api/seminar/file/upload?filename=files/${filename()}.${extension}`,
            {
              method: "POST",
              body: prop.value[0],
            },
  ...
  const onSubmit: SubmitHandler<seminarCreateOnSubmitSchemaType> = async (
    data: seminarCreateOnSubmitSchemaType,
  ) => {
  ...
  const onError: SubmitErrorHandler<seminarCreateOnSubmitSchemaType> = (
    errors,
  ) => {
    toast("エラーが発生しました。", {
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="text" placeholder="講習会名" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="text" placeholder="会場名" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
